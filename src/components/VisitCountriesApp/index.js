...
import VisitedCountryListItem from '../VisitedCountryListItem'
...
class VisitCountriesApp extends Component {
  ...
  onVisitCountry = id => {
    ...
    const updatedCountryList = countryList.map(eachCountry => {
  ...
  render() {
    const {countryList} = this.state
    const visitedCountryList = countryList.filter(
    ...
    return (
      <AppContainer>
        ...
        {visitedCountryList.length > 0 ? (
          <VisitedCountryList>
            {visitedCountryList.map(eachVisitedCountry => (
              <VisitedCountryListItem
                key={`visited-${eachVisitedCountry.id}`}
